...
const initialStore = {
    ...
    serviceAreas: await mongoArray('serviceAreas', DEFAULT_SERVICE_AREAS),
...
const serverServiceAreas = initialStore.serviceAreas?.length ? initialStore.serviceAreas : DEFAULT_SERVICE_AREAS;
...
const CANCELLATION_POLICY_CONFIG = {
    freeCancellationHoursBeforePickup: 24, // > 24h: 100% refund, 0% fee
    standardCancellationHoursBeforePickup: 6, // 6h - 24h: 20% fee
    lateCancellationFeePercent: 50, // < 6h: 50% fee
    standardCancellationFeePercent: 20,
    freeCancellationFeePercent: 0,
    gracePeriodMinutesAfterBooking: 60 // within 1h of booking: 100% refund
};
...
function calculateCancellationRefund(booking) {
    ...
    if (minutesSinceBooking <= CANCELLATION_POLICY_CONFIG.gracePeriodMinutesAfterBooking) {
        ...
        feePercent = CANCELLATION_POLICY_CONFIG.standardCancellationFeePercent;
        tier = 'next_day';
...
app.get('/api/vehicles/:id', (req, res) => {
    ...
    res.json({ success: true, vehicle: { ...vehicle, reviews } });
});
...
app.post('/api/vehicles', (req, res) => {
    ...
    if (!vehicleData.name || (!vehicleData.dailyPrice && !vehicleData.pricePerDay)) {
        return res.status(400).json({ success: false, error: 'Vehicle name and price are required.' });
    }
    ...
    if (idx >= 0) {
        if (serverVehicles[idx].ownerId !== user.id && serverVehicles[idx].owner?.id !== user.id) {
            return res.status(403).json({ success: false, error: 'Unauthorized to modify this vehicle.' });
        }
        serverVehicles[idx] = { ...serverVehicles[idx], ...newVeh };
    }
    else {
...
app.get('/api/bookings', (req, res) => {
    ...
    const userBookings = serverBookings
        .filter((b) => b.bookerId === user.id || (user.role === 'owner' && findVehicle(b.vehicleId)?.owner?.id === user.id))
        .map((b) => ({
        ...b,
...
app.get('/api/cancellation-policy', (req, res) => {
    res.json({
        ...
        description: 'Free cancellation (>24h before pickup: 100% refund). Standard cancellation (6h–24h before pickup: 20% fee). Late cancellation (<6h before pickup: 50% fee). Full security deposit is always refunded.'
    });
});
...
app.put('/api/users/profile', (req, res) => {
    ...
    const user = result.user;
    const { name, phone, avatar, role, bankAccount } = req.body;
    const updates = {};
