...
export const EditVehicleModal = ({
  ...
  const [pricePerDay, setPricePerDay] = useState(vehicle.dailyPrice || vehicle.pricePerDay || 1500);
  ...
  const [maxRentalDays, setMaxRentalDays] = useState(vehicle.maxRentalDays || 15);
  const [weeklyDiscountPercent, setWeeklyDiscountPercent] = useState(vehicle.weeklyDiscountPercent || 10);
  const [monthlyDiscountPercent, setMonthlyDiscountPercent] = useState(vehicle.monthlyDiscountPercent || 25);
  const [securityDeposit, setSecurityDeposit] = useState(vehicle.securityDeposit || 2000);

  // Hourly Rental (Separate)
  const [hourlyRentalEnabled, setHourlyRentalEnabled] = useState(vehicle.hourlyRentalEnabled || false);
  const [hourlyPrice, setHourlyPrice] = useState(vehicle.hourlyPrice || 180);
  const [minRentalHours, setMinRentalHours] = useState(vehicle.minRentalHours || 2);
  const [maxRentalHours, setMaxRentalHours] = useState(vehicle.maxRentalHours || 8);
  ...
  const [pickupInstructions, setPickupInstructions] = useState(vehicle.pickupInstructions || 'Key handover at apartment security gate.');
  ...
  const handleSave = (e) => {
    ...
    const finalMaxDays = (hasMaxDaysLimit && Number(maxRentalDays) > 0)
    ...
    const updated = {
      ...
      minRentalDays: Math.max(1, Number(minRentalDays)),
      ...
      minRentalHours: Math.max(1, Number(minRentalHours)),
      maxRentalHours: Math.max(1, Number(maxRentalHours)),
      isAvailable,
      instantBooking,
      location,
      pickupInstructions,
    ...
    // Save updates to real backend
    backendService.updateVehicle(vehicle.id, updated).then((res) => {
  ...
  const handleDelete = () => {
    ...
    backendService.deleteVehicle(vehicle.id).then(() => {
