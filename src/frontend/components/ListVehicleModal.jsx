...
export const ListVehicleModal = ({
  ...
  const [maxRentalHours, setMaxRentalHours] = useState(8);
  ...
  const [pickupAddress, setPickupAddress] = useState('Baner High Street, Baner, Pune');
  const [pickupInstructions, setPickupInstructions] = useState('Key handover at apartment security gate. Please call 15 minutes before arrival.');
  const [locationCoords, setLocationCoords] = useState({ lat: 18.5590, lng: 73.7868 });
  ...
  const handleNext = () => {
    ...
    if (step === 1) {
      if (!brand.trim() || !model.trim()) {
        setErrorText('Please enter vehicle make and model.');
        return;
      }
      if (!regNumber.trim()) {
    ...
    if (step === 2) {
      if (!pricePerDay || pricePerDay < 100) {
        setErrorText('Daily price must be at least ₹100.');
        return;
      }
    }
    if (step === 4) {
  ...
  const handleSubmit = (e) => {
    ...
    const newVehicle = {
      ...
      images: [], // MyRyedo does not require vehicle image uploads
      features: [
        'Verified Vehicle Registration',
        '24/7 Roadside Assistance',
        'Commercial Insurance Active',
        'Clean Sanitized Cabin',
        'FastTag Toll Enabled'
      ],
      specs: {
        mileageOrRange: fuel === 'Electric' ? '420 km range' : '18.5 km/l',
        engineOrBattery: fuel === 'Electric' ? 'High Density Battery' : 'Standard Engine',
        topSpeed: '140 km/h'
      },
      verified: true,
      verificationStatus: 'verified',
      description: description.trim() || `Clean and well-maintained ${brand.trim()} ${model.trim()} available for flexible daily and hourly rental.`,
      isAvailable,
      instantBooking,
      dailyRentalEnabled,
    ...
    // Save directly to persistent backend
    backendService.createVehicle(newVehicle).then((res) => {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="bg-[#F8FAFC] rounded-2xl p-4 border border-gray-100 space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {fuel} · {transmission} · {seats} Seats · {regNumber || 'Verified Plate'}
                    </p>
                  </div>
                </div>
          ...
          <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
            ...
            {step < 6 ? (
              ...
              <button
                type="submit"
                className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black flex items-center gap-2 shadow-lg shadow-emerald-600/20 transition-all cursor-pointer"
              >
