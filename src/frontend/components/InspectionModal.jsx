...
export const InspectionModal = ({
  ...
  const [odometer, setOdometer] = useState(inspectionType === 'return' ? 14650 : 14200);
  const [fuelLevel, setFuelLevel] = useState(inspectionType === 'return' ? 95 : 100);
  const [notes, setNotes] = useState(
    inspectionType === 'pickup' 
      ? 'Vehicle exterior and interior verified clean. Tires, spare tire and original toolkit inspected.' 
      : 'Returned in good condition. Fuel level checked against pickup handover.'
  );
  const [checklist, setChecklist] = useState({
    cleanInterior: true,
    noVisibleDents: true,
    rcInsurancePresent: true,
    spareTireToolkit: true,
    lightsAndIndicators: true
  });
