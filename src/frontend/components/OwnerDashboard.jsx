...
export const OwnerDashboard = ({
  ...
  const hostVehicles = currentUser 
    ? vehicles.filter(v => v.owner?.id === currentUser.id || (currentUser.email && v.owner?.email === currentUser.email))
    : [];
                  ...
                  <div className="flex items-center gap-4 self-end sm:self-center">
                    <div className="text-right">
                      <span className="text-sm font-black text-[#FF6400]">
                        ₹{(b.basePrice || 1800).toLocaleString('en-IN')}
                      </span>
